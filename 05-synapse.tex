...
data without a coherent, manageable architecture. We
...
Neo4j, and Elasticsearch. We and others have built over a dozen microservices
...
Developers specify what data to share among services within model declarations through \synapse's intuitive API.
Models are expressed in
terms of high-level objects that are
...
\bottomrule
 ...
 {\code {\footnotesize add\_read\_deps,}}    & Specify explicit dependencies for read and  \\
 {\code {\footnotesize add\_write\_deps}}    & write DB queries.                           \\ \midrule
 {\code {\footnotesize delivery\_mode}}      & Parameter for selecting delivery semantic.  \\ \midrule
...
\begin{lstlisting}[xleftmargin=4pt,framexleftmargin=2pt]
# Publisher side (Pub1).
class User
...
\S\ref{sec:examples}. 
Other API calls that can be
 used by subscribers are
{\code {\small bootstrap?}} and {\code {\small delivery\_mode}},
\begin{wrapfigure}{r}{0.18\textwidth}
\vspace{0.55em}
\begin{minipage}{.18\textwidth}
\begin{lstlisting}[xleftmargin=4pt,framexleftmargin=2pt]
...
\begin{center}
\includegraphics[width=3cm]{figures/synapse/dep-graph.pdf}
\end{center}
\end{minipage}
\end{tabular}\hfill
\begin{tabular}{c}
...
\heading{Scaling the Version Store.} \label{sec:arch:hashing}
...
{\bf Controllers} & {\bf (of 170k)} & \multicolumn{2}{c|}{{\bf Messages}} & \multicolumn{2}{c|}{{\bf per Message}}  & \multicolumn{2}{c|}{{\bf Time (ms)}}  & \multicolumn{2}{c}{{\bf Time (ms)}} \\ \hline
    ...
    \subfigure[c][{\bf \synapse Overheads in Real Applications}]{
     \includegraphics[width=6cm]{figures/synapse/overhead-hist.pdf}
...
\F~\ref{tab:crowdtap-overheads} shows our results. In total, 55 controllers
