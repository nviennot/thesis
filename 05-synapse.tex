\chapter{\synapse: Distributed Record-Replay for Database Systems}
\label{ch:synapse}
...
implement data replication protocols often run an underlying record-replay
mechanism.
...
Data replication can be seen as recording execution of a primary DB and
replaying execution on a replica DB.
For example, many distributed databases such
as MongoDB, RethinkDB or Elasticsearch provide a log based replication mechanism,
which is a form of record-replay.  The target audience of these
...
\synapse shares many similarities with the deterministic record-replay system \scribe.
...
Despite their similarities, the two systems are not compatible in the features
they provide.

Could \scribe, or even \dora which provides a superset of \scribe
functionalities, be sufficient to implement a heterogeneous DB replication
...
\synapse's key insight is the use of ORM as data translation layer. Mutable
