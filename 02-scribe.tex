...
Table~\ref{tab:events} lists all
...
Section~\ref{sec:syscalls} describes the {\em syscall\_ret} and {\em
  copy\_data} events used for system calls and data transfer between
kernel and user space,
respectively.
Section~\ref{sec:memory} describes the {\em page\_public}, {\em
  page\_share\_read}, and {\em page\_own\_write} events used for
shared memory interleavings.  Section~\ref{sec:rendezvous} covers
the {\em rendezvous} event used for ordering access to shared
resources via system calls. Section~\ref{sec:sync} describes the {\em
...
calls, then describe in Section~\ref{sec:rendezvous} how \scribe{}
...
For example, Figure~\ref{fig:recplay} shows the recording of
...
in Section~\ref{sec:rendezvous}.  When a replayed process invokes a
...
Figure~\ref{fig:recplay} shows the replay of the \code{gettimeofday} 
...
key challenge, and is discussed in Section~\ref{sec:sync}.
...
Figure~\ref{fig:rendezvous} shows the recording of
...
Section~\ref{sec:results} shows that our approach incurs low overhead
...
Figure~\ref{fig:rendezvous} illustrates the use of rendezvous points
when replaying the \code{write} and \code{read} system calls.

  

Table~\ref{tab:rendezvous} lists all categories of related systems
...
Table~\ref{tab:rendezvous} are not mutually exclusive.  For example,
...
memory, described in Section~\ref{sec:memory}.
...
described in Section~\ref{sec:memory}, a process requesting access to
...
Figure~\ref{fig:recasync}a illustrates record-replay of signals.
...
necessary, give up ownership. Figure~\ref{fig:recasync}b illustrates
...
to be low in practice.  Section~\ref{sec:results} presents
...
listed in
Table~\ref{tab:scenarios}.  The list includes (1)~server applications
...
listed in Table~\ref{tab:scenarios}.  Applications were all run with
...
Figure~\ref{fig:overhead} shows the performance overhead of recording
...
Figure~\ref{fig:overhead} also shows the performance of replaying the
...
Figure~\ref{fig:storage} shows the storage growth rate of recording.
...
Figure~\ref{fig:totals} shows the average number of processes and
...
events, as shown in Figure~\ref{fig:storage}.  The threads in {\tt
  ssh-c} are
...
Figure~\ref{fig:syncpts} shows the time interval between consecutive
...
Figure~\ref{fig:syncpts} also shows the average length of sync points
...
Figure~\ref{fig:stats} shows the total number of signals and shared
...
Observe in Figure~\ref{fig:stats} that asynchronous events due to
...
Figure~\ref{fig:delays} shows the amount of delay incurred for signals
