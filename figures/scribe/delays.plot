...
#
# new plot
set output "delays2.eps"
set multiplot

newavg(avg,inst,defer) = (avg * defer) / (inst + defer) 

set key 18,90000
plot 'delays.dat'  index 1 using 1:4  title "signal (max)" lt 4, \
	"" index 1 using 1:3 title "signal (defer)" lt 2, \
	"" index 1 using 1:(newavg($3,$5,$6)) title "signal (avg)" lt 3

set key 38,90000
plot 'delays.dat'  index 1 using 2:8 title "memory (max)" lt 1 fs solid 0.4, \
	"" index 1 using 2:7 title "memory (defer)" lt 2 fs solid 0.4, \
	"" index 1 using 2:(newavg($7,$9,$10)) title "memory (avg)" lt 3 fs solid 0.4
