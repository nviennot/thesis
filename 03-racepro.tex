...
\racepro heavily relies on record-replay techniques to perform its tasks.
...
effect of the modification. Even though reordering system call can be seen as a
...
\racepro's record-replay functionality builds on \scribe.
...
traditional deterministic record-replay presented in the previous chapter with
...
\scribe only supports traditional deterministic record-replay (i.e. perfectly
reenacting a previously recorded execution), enhancements had to be made to
...
a group of processes together.

\scribe made significant efforts to only record non determinism data in order to
provide the best recording performance. However, additional information must be
recorded for \racepro to perform its tasks. To calculate the happens-before
graph and perform race detection, system call semantics must be extracted and
analyzed from the recording. In addition to what \scribe records, \racepro needs
additional information in three areas to perform its tasks.
First, when \scribe recorded application calls a system call,
neither the system call number, nor the return value are recorded in the log file.
This information is deterministic and thus provide no benefit for deterministic
replay. Because \racepro needs to extract system call semantics to perform race
analysis, such values are recorded in the log file.
For example, \racepro needs to identify the \code{wait()} calls among all the
system calls in the recording, and access their return values to identify the
corresponding PIDs to perform wait-wakeup race detection.
...
However, \racepro needs to identify each resource to be able to relate
system calls accessing the same resource. For this, resource type and unique resource
identifiers are recorded along side to the sequence number of each rendezvous
points. A complication arise with pipes and sockets as the kernel considers each
endpoint to be a separate resource for performance. Because \racepro needs to
...
to replay them verbatim while ignoring the original source of the signal (e.g.
calls to \code{kill()})~(\S\ref{scribe:sec:signaldelivery}). However, \racepro needs to
tie the signal delivery with its corresponding source. For this, a signal
identifier is recorded in a signal emission event when a signal emitted by a source process,
and recorded again in the signal delivery event by the target process.
This allow \racepro to bind the two events, sending and
receiving, of a signal delivery mechanism to perform signal race detection.
...
record a minimal log file containing only non-deterministic data akin to \scribe,
and replay it once to recover all the required deterministic information.

\racepro's explorer manipulates the recorded log file to finely control the
...
can be seen as a large list of serialized events. When processing a log file,
...
processing. The stream starts with a source, emitting each event from a given
...
sink, which can write a new log to disk, or feed events directly to the
...
all sequence numbers of a given resource due to the reordering of system calls.
...
stream, it waits for all other processes to also reach their barrier.
The last process to reach its barrier resumes the execution of the other
processes. This feature is useful in certain edge cases with the wait-wakeup
race, where multiple {\code exit()} calls sharing no common resource must be
synchronized. Barrier can also be useful in detecting \toctou races as the
replayer can invoke callbacks at a barrier right before resuming execution.
Such callback could modify a file. Finally, barriers are used to ensure all
...
The replayer remains mostly with the same feature set, and provides no tolerance
to execution divergences, which is why \racepro must go-live as soon as the
modified behavior has been executed. To improve the quality of race triage, it
would be desirable to make the replayer tolerant to execution divergence.
Allowing the replayed execution, even though modified, to continue replaying
after the modifications introduced by \racepro may provide a quality automatic
checker.
In cases where the exhibited race is benign, a minimal divergence is expected,
but when the race is harmful, a substantial divergence would be expected.
...
This ability goes beyond deterministic record-replay and is instrumental to
