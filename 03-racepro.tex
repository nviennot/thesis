\section{Pervasive Detection of Process Races in Deployed Systems}
\label{ch:racepro}
...
We present the first study of real process races and the first system
designed to detect them.  Our study of hundreds of applications shows
that process races are numerous, difficult to debug, and a real threat
to reliability.  
To address this problem, we 
created \racepro, a system for automatically detecting these races.
\racepro checks deployed systems \emph{in-vivo} by recording live
executions then deterministically replaying and checking them later.
This approach increases checking coverage beyond the configurations or
executions covered by software vendors or beta testing sites.  \racepro
records multiple processes, detects races in the recording among
system calls that may concurrently access shared kernel objects, then
tries different execution orderings of such system calls to determine
which races are harmful and result in failures.  To simplify race
detection, \racepro models under-specified system calls based on load and
store micro-operations.  To reduce false positives and negatives, \racepro
uses a \emph{replay and go-live} mechanism to distill harmful races
from benign ones.  We have implemented \racepro in Linux, shown that it
imposes only modest recording overhead, and used it to detect a number
of previously unknown bugs in real applications caused by process races.
...
\code{ps | grep X} race (\S\ref{sec:detect}).  Similarly, a
...
This paper is organized as follows.  \S\ref{sec:study} presents a
study of process races and several process race examples.
\S\ref{sec:overview} presents an overview of the \racepro architecture.
\S\ref{sec:record} describes the execution recording mechanism.
\S\ref{sec:detect} describes  the system call modeling using
micro-operations and the race detection algorithm.
\S\ref{sec:validate} describes how replay and go-live are used to
determine harmful races.  \S\ref{sec:eval} presents experimental
results.  \S\ref{sec:related} discusses related work.  Finally, 
\S\ref{sec:conclusion} presents some concluding remarks and directions
for future work.

\subsection{Process Race Study} \label{sec:study}
...
\S\ref{sec:findings} presents our findings. \S\ref{sec:example}
describes four process race examples from the most serious to the
least. 

\subsubsection{Findings} \label{sec:findings}

Table~\ref{tab:data} summarizes the collected pages and bugs;
...
\end{tabular}
\caption{{\em Summary of collected pages and bugs.}} \label{tab:data}
\end{table}
...
did not include them in Table~\ref{tab:data}.

\begin{figure}[t]
  \hskip 5mm
  \includegraphics[width=0.45\linewidth,angle=270]{figures/racepro/race-effects}
  \vskip -5mm
  \caption{\em Process races breakdown by effects.} \label{fig:effects}
  \vskip -2mm
\end{figure}
...
\centering
\vskip -4mm
\includegraphics[width=.30\linewidth]{figures/racepro/race-packages}
\includegraphics[width=.30\linewidth]{figures/racepro/race-processes}
\includegraphics[width=.30\linewidth]{figures/racepro/race-languages}
\vskip -1mm
\caption{{\em Process races breakdown.} X axis shows the number of software
  ...
  command in a script causes a \code{fork}.)} \label{fig:breakdown}
\end{figure*}
...
in infinite loops. Figure~\ref{fig:effects} summarizes the effects of
all process races from Table~\ref{tab:data}.
...
replacement of System V \code{init} scripts.  Figure~\ref{fig:breakdown}
breaks down the process races by packages, processes, and
programming languages involved.  Over half of the \nprace process races,
including all examples described in \S\ref{sec:example}, require 
...
\S\ref{sec:example}, occur only due to site-specific software,
hardware, and user configurations.  Moreover, many of the sampled
process races, including all of those described in
\S\ref{sec:example}, occur only due to rare runtime factors. For
...
such as Bug 1, 3, and 4 in \S\ref{sec:example},
...
races, such as Bug 2 in \S\ref{sec:example}. 
...
\subsubsection{Process Race Examples} \label{sec:example}
...
\end{footnotesize}
    \vskip -2mm
    \caption{{\em dash-MySQL race.}} \label{fig:dash-mysql}
  \end{minipage}
\end{minipage}
\begin{minipage}{0.49\linewidth}
    \centering
...
\end{footnotesize}
      \vskip -2mm
      \caption{{\em bash race.}} \label{fig:bash}
    \end{minipage}
\end{minipage}
\vskip -2mm
\end{figure*}
...
\begin{minipage}{0.48\linewidth}
  \centering
  \includegraphics[width=\linewidth]{figures/racepro/flow}
  \vspace{-7mm}
  \caption{{\em \racepro Workflow.} Thin solid lines represent recorded
    ...
    explore.} \label{fig:flow}
\end{minipage}
\hfill
\begin{minipage}{0.47\linewidth}
  \centering
  \includegraphics[width=\linewidth]{figures/racepro/arch}
  \vspace{-5.5mm}
  \caption{{\em \racepro Architecture.} Components are shaded. The recorder
    ...
    executions are stored in files.} \label{fig:arch}
\end{minipage}
\end{figure*}
...
Figure~\ref{fig:dash-mysql} shows the event sequence causing this race.
...
user-specified size.  Figure~\ref{fig:bash} shows this problematic
...
workflow shown in Figure~\ref{fig:flow}.  
...
Figure~\ref{fig:arch} shows the \racepro architecture used to support its
...
(\S\ref{sec:record}) and detects (\S\ref{sec:detect}) and validates
(\S\ref{sec:validate}) races using these components.  

\subsection{Recording Executions} \label{sec:record}
...
high-level program semantics~(\S\ref{sec:detect}).  Finally, \racepro's
...
Table~\ref{tab:resources} lists the kernel objects tracked by \racepro.
...
\end{tabular}
\caption{{\em Shared kernel objects tracked.}} \label{tab:resources}
\end{table}
...
all modify a common resource~(\S\ref{sec:detect}).  One complication 
...
\subsection{Detecting Process Races} \label{sec:detect}
...
recorded execution (\S\ref{sec:graph}).  To determine whether a set of
...
(\S\ref{sec:model}) on those resources.  \racepro then runs a set of
happens-before based race detection algorithms to detect load-store
and wait-wakeup races (\S\ref{sec:potential}).

\subsubsection{The Happens-Before Graph} \label{sec:graph}
...
\racepro's validation step (\S\ref{sec:validate}) and will not be reported.

\begin{figure}[t]
\centering
\includegraphics[width=1.0\linewidth]{figures/racepro/psgrep}
\vspace{-6mm}
\caption{{\em The Happens-before graph and respective vector-clocks
  ...
  involved. The \code{read} of process $P_2$ and the \code{execve} of
  $P_3$ form a load-store race~(\S\ref{sec:load-store}), and so do the
  ...
  wait-wakeups race~(\S\ref{sec:wait-wakeup}). For clarity, not all
  system calls are shown.} \label{fig:psgrep}
\vspace{-0mm}
\end{figure}

Figure~\ref{fig:psgrep} shows the happens-before graph for the example
...
\subsubsection{Modeling Effects of System Calls} \label{sec:model}
...
\vskip -3mm
\label{tab:syscall-effects}.
\end{table}

Table~\ref{tab:syscall-effects} shows the template of micro-operations
...
\subsubsection{Race Detection Algorithms} \label{sec:potential}
...
load-store races (\S\ref{sec:load-store}), wait-wakeups races
(\S\ref{sec:wait-wakeup}), and wakeup-waits races
(\S\ref{sec:wakeup-wait}).  \racepro may also be extended to detect
other types of races (\S\ref{sec:multilevel}).

\paragraph{Load-Store Races} \label{sec:load-store}
...
example shown in Figure~\ref{fig:psgrep}, the system calls
...
lockset because processes rarely use standard locks (\S\ref{sec:study}). 
...
\racepro detects and later validates (\S\ref{sec:validate}) races at the
...
\paragraph{Wait-Wakeups Races} \label{sec:wait-wakeup}
...
In the \code{ps | grep X} example shown in Figure~\ref{fig:psgrep},
...
\end{footnotesize}
\vspace{-5mm}
\center{\scriptsize \bf (b)}
\end{minipage}
\vspace{-1mm}
\caption{{\em Wait-wakeups races in streams.}} \label{fig:streams}
\vspace{-1mm}
\end{figure}

The relative order of the wakeup system calls may matter if their
effect on the resource is cumulative. For instance,
Figure~\ref{fig:streams} depicts a cumulative wait-wakeups scenario
...
In Figure~\ref{fig:streams}a, a blocking \code{read} occurs after two 
\code{write}s and consumes their cumulative data.  However, in
Figure~\ref{fig:streams}b, the \code{read} occurs before the second
\code{write} and returns the data only from the first \code{write}. 
Note that $S_2$ and $S_3$ in Figure~\ref{fig:streams}a do not form a
...
\paragraph{Wakeup-Waits Races} \label{sec:wakeup-wait}
...
\paragraph{Many-System-Calls Races} \label{sec:multilevel}
...
(\S\ref{sec:reexec}). 
\subsection{Validating Races}  \label{sec:validate}
...
be harmful and lead to a failure (\S\ref{sec:eval}).
...
execution~(\S\ref{sec:branches}). It replays the modified 
execution until the race occurs, then makes the execution
go-live~(\S\ref{sec:reexec}).  It checks the live execution for
failures~(\S\ref{sec:check}), and, if found, reports the race as a 
bug.

\subsubsection{Creating Execution Branches}  \label{sec:branches}
...
race in Figure~\ref{fig:psgrep} is $[3,3,2]$, and the consistent cut
...
divergence~(\S\ref{sec:reexec}).  Instead, \racepro only considers
...
the execution scenario depicted in Figure~\ref{fig:reorder}, which
...
\subsubsection{Replaying Execution Branches and Going Live}  \label{sec:reexec}
...
\end{footnotesize}
\end{minipage}
\caption{\em Replay divergence due to reordering.} \label{fig:reorder}
\vskip -2.5mm
\end{figure}
...
calls (\S\ref{sec:multilevel}). Since it already records the go-live
...
Figure~\ref{fig:reorder} in which the system calls $S_1$ and
...
with respect to $S_4$. Figure~\ref{fig:diverge}a illustrates the
...
\racepro.  Figure~\ref{fig:diverge}b illustrates one such scenario where
...
\end{verbatim}
\end{footnotesize}
    \center{\vskip -3mm \hskip -5mm \bf (a)}
  \end{minipage}
  \begin{minipage}{.49\linewidth}
\begin{footnotesize}
\begin{verbatim}
...
\end{verbatim}
\end{footnotesize}
    \center{\vskip -3mm \hskip -5mm \bf (b)}
  \end{minipage}
  \caption{{\em Replay divergence examples.}} \label{fig:diverge}
\end{minipage}
\vskip -3mm
\end{figure}
...
different one. For example, in Figure~\ref{fig:diverge}b the race
...
\subsubsection{Checking Execution Branches}  \label{sec:check}
...
(\S\ref{sec:bug}), it may miss domain-specific ``silent''
...
\caption{{\em Bug detection statistics.}  {\bf Processes} is the
  ...
  races, and {\bf Harmful} harmful races that led to failures.  {\bf
    Record} and {\bf Replay} are the times to record and replay the
  executions, respectively. {\bf Generate} is the average time to
  generate an execution branch and {\bf Validate} the average time to
  validate a race. } \label{tab:bug-stat}

\vskip -0mm
\end{table*}

\subsubsection{Bugs Found} \label{sec:bug}
...
bugs in \S\ref{sec:study} are in programs that we cannot compile, so we
...
Table~\ref{tab:bugs} shows all the bugs \racepro found.  \racepro found a
...
\subsubsection{Bug Statistics} \label{sec:race}

Table~\ref{tab:bug-stat} shows various statistics for each detected
...
may occur in a subsection of the code that is executed multiple times,
...
Second, a bug such as missing locks around critical subsections may
...
bugs listed in Table~\ref{tab:bug-stat}.  These measurements were done
on an HP DL360 G3 server with dual 3.06~GHz Intel Xeon CPUs, 4~GB RAM,
and dual 18~GB local disks.  For
each bug, Table~\ref{tab:bug-stat} shows the time to record the
...
evaluation described in this paper.  In the remainder of this subsection, we
...
However, as discussed in \S\ref{sec:intro}, existing systems for detecting
